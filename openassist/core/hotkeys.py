...
class NativeHotkeyThread(threading.Thread):
    ...
    def run(self):
        ...
        # AGGRESSIVE CLEANUP: Flush all potential IDs (100-150)
        for i in range(100, 150):
            user32.UnregisterHotKey(None, i)
        
        current_id = 100
        for action, key_str in self.manager.config.get("hotkeys", {}).items():
            if action.startswith("move_") or action.startswith("scroll_"): continue
            ...
            if vk:
                ...
                else: logger.warning(f"⚠️ Native Hotkey Failed: {action}")
                current_id += 1
...
class HotkeyManager:
    def __init__(self, config, app):
        ...
        self.signals.started.connect(lambda n: self.app.start_move(n.replace("move_","")) if n.startswith("move_") else None)
        self.signals.stopped.connect(lambda n: self.app.stop_move() if n.startswith("move_") else None)
    ...
    def _cleanup_stuck_keys(self):
        ...
        if stuck:
            for k in stuck: del self.active_keys[k]
            self.app.stop_move()
    ...
    def _handle_native_trigger(self, hotkey_id):
        ...
        if action: self.signals.triggered.emit(action)
    ...
    def _on_press(self, key):
        ...
        for action, key_str in self.config.get("hotkeys", {}).items():
            if not (action.startswith("move_") or action.startswith("scroll_")): continue
            req = self._parse_key_pynput(key_str)
            if req and all(any(rk in self.active_keys for rk in group) for group in req):
    ...
    def _normalize_key(self, key):
        if isinstance(key, keyboard.Key): return str(key).lower()
        if isinstance(key, keyboard.KeyCode):
            if key.vk is not None: return f"vk_{key.vk}"
    ...
    def _parse_key_native(self, key_str):
        ...
        VK_MAP = {
            ...
            "pageup": 0x21, "pagedown": 0x22, "home": 0x24, "end": 0x23, "ins": 0x2D, "del": 0x2E,
            "[": 0xDB, "]": 0xDD, "\\": 0xDC, "/": 0xBF, "`": 0xC0, "m": 0x4D, "a": 0x41, "s": 0x53, "z": 0x5A, "n": 0x4E, "e": 0x45,
        ...
        for p in parts:
            p = p.strip()
            if p == "ctrl": m |= MOD_CON
            elif p == "shift": m |= MOD_SHF
            elif p == "alt": m |= MOD_ALT
            elif p == "win": m |= MOD_WIN
            elif p in VK_MAP: v = VK_MAP[p]
            elif len(p) == 1: v = ord(p.upper())
        return m, v
    ...
    def _parse_key_pynput(self, s):
        ...
        for p in parts:
            p = p.strip()
            if p == "ctrl": groups.append({"key.ctrl", "key.ctrl_l", "key.ctrl_r"})
            elif p == "shift": groups.append({"key.shift", "key.shift_l", "key.shift_r", "key.shift", "key.shift_r"})
            elif p == "up": groups.append({"key.up"})
            elif p == "down": groups.append({"key.down"})
            elif p == "left": groups.append({"key.left"})
            elif p == "right": groups.append({"key.right"})
            elif len(p) == 1: groups.append({p, f"vk_{ord(p.upper())}"})
        return groups
